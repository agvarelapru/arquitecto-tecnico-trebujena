...
$conexion=mysqli_connect(DBHOST,DBUSER,DBPASS,DBNAME) or
  ...
  $where="";
    ...
    if($_SESSION['direccion']!=""){
      $where.=" encargos_direccion LIKE '%".$_SESSION['direccion']."%' ";

    }

    if($_SESSION['trabajo']!=""){
      if($where==""){
          $where.=" encargos_tipo LIKE '%".$_SESSION['trabajo']."%' ";
      }else{
        $where.=" and encargos_tipo LIKE '%".$_SESSION['trabajo']."%' ";
      }

    }
    if($_SESSION['fechaEncargo']!=""){
      if($where==""){
          $where.="  encargos_fecha >= '".$_SESSION['fechaEncargo']." 00:00:00' ";
      }else{
        $where.="  and encargos_fecha >= '".$_SESSION['fechaEncargo']." 00:00:00' ";
      }
...
if($_SESSION["perfil"]=="Administrador" || $_SESSION["perfil"]=="Tecnico"){
  ...
  $rs_contactos = mysqli_query($conexion, "select * from encargos where ".$where);
  $num_total_registros = mysqli_num_rows($rs_contactos);
}else{
  $rs_contactos = mysqli_query($conexion, "select * from encargos where ".$where." and encargos_usuario='".$_SESSION['id']."'");
  $num_total_registros = mysqli_num_rows($rs_contactos);
...
if($_SESSION["perfil"]=="Administrador" || $_SESSION["perfil"]=="Tecnico"){
  ...
  $registros=mysqli_query($conexion,"select *  from encargos where".$where." order by encargos_fecha  DESC LIMIT ".$inicio."," . $TAMANO_PAGINA) or
    die("Problemas en el select:".mysqli_error($conexion));
}else{
  $registros=mysqli_query($conexion,"select *  from encargos where".$where." and encargos_usuario='".$_SESSION['id']."' order by encargos_fecha  DESC LIMIT ".$inicio."," . $TAMANO_PAGINA) or
    die("Problemas en el select:".mysqli_error($conexion));
}
  ...
  $registros2=mysqli_query($conexion,"select *  from usuarios where usuarios_id='".$reg["encargos_usuario"]."'") or
    die("Problemas en el select:".mysqli_error($conexion));
  $nombreUsuario="";
...
if($_SESSION["perfil"]=="Administrador"){
  echo"<form class='form-horizontal'  action='?p=encargos/borrarencargos' method='post'>";
  echo "<div style='float:left;margin-top:3%; margin-right:0%; z-index:1;width:1.5%;'>";
  echo "<input class='form-control' type='checkbox' name='tic[]' id='tic' value='".$reg['encargos_id']."'>";
  echo "</div>";}
  echo "<div class='list-group' style='width:88%; margin-left:6%;'>";

    echo "<a href='?p=encargos/mostrarencargo&encargos_id=".$codigo."' class='list-group-item'>";
  ...
  if($_SESSION["perfil"]=="Administrador"){
  ?><button class="contact submit btn btn-primary btn-xl" style="float:right;" data-toggle="modal" data-target="#myModal"  type="button" name="eliminar" id="eliminar" value="Borrar presupuestos" >Borrar encargos</button>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" >
          <p style="color:black;">¿Esta seguro de eliminar los encargos?</p>
  ...
  ?>
    ...
    <form class='form-horizontal'  action='?p=encargos/verencargos' method='post'>
