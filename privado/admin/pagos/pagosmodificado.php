...
if (!preg_match("/^[0-9,.]*$/",$pago)) {
  $pagoErr = "Solo numeros y ,. ";
     ...
     $consulta_mysql3=mysqli_query($conexion,"select * from pagos where pagos_id='".$_POST["pago_id"]."'") or
     die("Problemas en el select:".mysqli_error($conexion));

     while($reg3=mysqli_fetch_array($consulta_mysql3)){
     $pagosantiguo=$reg3["pagos_cantidad"];
   }
...
$encargosPagoNuevo=$encargoPagado-$pagosantiguo;
$falta=$encargoHonorarios-$encargosPagoNuevo;
       if($_POST["pago"]>$falta){
...
$pagado=$_POST["pagado"];
$pago=$_POST["pago"];
$totalresta=$pagado-$pagosantiguo;

$totalnuevo=$totalresta+$pago;

mysqli_query($conexion, "update pagos set pagos_cantidad='$pago', pagos_observaciones='$_REQUEST[observaciones]' where pagos_id='$_REQUEST[pago_id]'") or die("Problemas en el select:".mysqli_error($conexion));


                mysqli_query($conexion, "update encargos set encargos_pagos='$totalnuevo' where encargos_id='$_REQUEST[encargo]'") or die("Problemas en el select:".mysqli_error($conexion));
...
<li><label for="pagodo" >Pagado:</label><?php echo " ".$totalnuevo." €";?><span class="error"><?php echo "  ".$pagadoErr;?></span></li>
...
<div class="alert alert-success alert-dismissible fade in">
    ...
    <strong>¡Bien!</strong> El pago se a modificado correctamente.
  </div>
<?php
}else if($pagadoErr!=""){
...
<div class="alert alert-danger alert-dismissible fade in">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>¡Upsss!</strong> El pago no se a modificado. Ha sobrepasado los honorarios.
  </div>
<?php
...
<div class="alert alert-danger alert-dismissible fade in">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>¡Upsss!</strong> El pago no se modifico.
  </div>
